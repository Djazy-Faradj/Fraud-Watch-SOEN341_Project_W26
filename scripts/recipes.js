 ...
 * recipes.js
 * -----------------------------------------------------------------------------
 * Responsibility: Recipe Management (Sprint 2)
 *
 * What this file does:
 * - Session guard (redirect to login if not authenticated)
 * - Load + render recipes from Supabase
 * - Search + filter (max cost)
 * - CRUD: Create/Edit/Delete recipes
 *
 * Data contract (Supabase tables used here):
 * Table: recipes
 * - id (int8)                   : primary key (auto-generated by DB)
 * - created_at (timestamptz)    : timestamp (set by DB)
 * - name (text)                 : recipe name/title
 * - creator (uuid)              : owner user id (FK -> users.id)
 * - ingredients (text)          : ingredient list (stored as plain text)
 * - steps (text)                : instructions (stored as plain text)
 * - cost (float4)               : numeric cost used for "max cost" filter
 * - allergens (text)            : allergen info (stored as plain text)
 * - dietary_tags (text[])       : array of tags (ex: ["vegan","halal"])
 * - time (int4)                 : time in minutes
 *
 * Relationships:
 * - recipes.creator references users.id (and users.id maps to auth.users.id)
 */
